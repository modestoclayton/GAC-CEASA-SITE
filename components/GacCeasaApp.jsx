...
function EntregasTab({ cadastros, transacoes, persistTransacoes, showToast, soMeuNome, setRecibo }) {
  ...
  const [editandoId, setEditandoId] = useState(null);
  const [entreguesAberto, setEntreguesAberto] = useState(false); // cascata: fechada por padrão
  ...
  // "Entregues" mostra tudo que já foi confirmado, mais recente primeiro —
  // fica escondido atrás de um "cascata" fechado por padrão, então nada
  // "some": só fica agrupado numa seção que você abre quando quiser conferir.
  const confirmadas = comEntrega
    .filter((v) => v.entrega.confirmada)
    .sort((a, b) => (b.entrega.dataConfirmacao || b.data || "").localeCompare(a.entrega.dataConfirmacao || a.data || ""));
      ...
      v.id === vendaId
        ? {
            ...
            entrega: {
              ...v.entrega,
              confirmada: next,
              dataConfirmacao: next ? todayISO() : null,
            },
          }
        : v
    );
        ...
        <>
          ...
          >
            ...
            <ChevronRight
              ...
              style={{
                color: C.inkSoft,
                transform: entreguesAberto ? "rotate(90deg)" : "rotate(0deg)",
          ...
          {entreguesAberto && (
            <div className="flex flex-col gap-2 mb-2">
              {confirmadas.length === 0 ? (
                ...
                confirmadas.map((v) => <EntregaCard key={v.id} v={v} />)
              )}
            </div>
          )}
        </>
